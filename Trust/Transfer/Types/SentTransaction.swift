...
extension SentTransaction {
    static func from(transaction: SentTransaction) -> Transaction {
        return Transaction(
            ...
            shardingFlag: transaction.original.shardingFlag.description,
            systemContract: transaction.original.systemContract.description,
            via: transaction.original.via?.description ?? "",
            date: Date(),
